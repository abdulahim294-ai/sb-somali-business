...
export const DialogOverlay = React.forwardRef<
  React.ElementRef<typeof DP.Overlay>,
  ...
  <DialogPortal>
    ...
    <DP.Content
      ...
      className={cn(
        ...
        "w-[calc(100%-2rem)] max-w-lg bg-background p-6 shadow-2xl rounded-2xl border border-border/60",
        "data-[state=open]:animate-in data-[state=closed]:animate-out",
        "data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0",
    ...
    >
      {children}
      <DP.Close className="absolute right-4 top-4 rounded-md p-1 opacity-60 hover:opacity-100 hover:bg-slate-100 transition-all focus:outline-none focus-visible:ring-2 focus-visible:ring-ring">
        <X className="h-4 w-4" />
...
export const DialogTitle = React.forwardRef<
  React.ElementRef<typeof DP.Title>,
  React.ComponentPropsWithoutRef<typeof DP.Title>
>(({ className, ...props }, ref) => (
  <DP.Title
    ref={ref}
    className={cn(
      "text-lg font-bold leading-none tracking-tight font-display text-slate-900",
      className
    )}
    {...props}
  />
